...
function addMissingArguments(j, mapping, name, args, paramsDescription) {
  var ary = common.getMethodAry(mapping, name) || 1;
  if (ary <= args.length) {
    return args;
  }
  var paramNames = paramsDescription.map(paramName);
  var tmpArgs = _.clone(args);
  var newArgs = _.compact(mapRight(_.take(paramsDescription, ary), function(paramDescription, index) {
  ...
  var nbMissingArgs = Math.max(0, ary - newArgs.length);
  var value = stringToASTNode(j, 'null');
  return newArgs.concat(_.times(nbMissingArgs, _.constant(value)));
}
